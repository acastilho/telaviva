  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // Keep status-only error and never expose an upstream body.
      }
...
export const schedulingClient = {
  async listActive(): Promise<ScheduledStream[]> {
    return request<ScheduledStream[]>('/streams/active')
  },

  async listUpcoming(creatorId?: string): Promise<ScheduledStream[]> {
  ...
  async create(payload: StreamCreatePayload, accessToken: string): Promise<ScheduledStream> {
    return request<ScheduledStream>('/streams', {
      method: 'POST',
      body: JSON.stringify(payload),
    }, accessToken)
  },

  async activate(streamId: string, roomId: string, accessToken: string): Promise<ScheduledStream> {
  ...
  async finish(streamId: string, accessToken: string): Promise<ScheduledStream> {
    return request<ScheduledStream>(`/streams/${encodeURIComponent(streamId)}/finish`, {
      method: 'POST',
    }, accessToken)
  },
