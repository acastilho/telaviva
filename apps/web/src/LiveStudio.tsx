...
export function LiveStudio({ onClose, streamId, accessToken }: LiveStudioProps) {
  ...
  const startBroadcast = async () => {
    ...
    if (usesRemoteSchedulingApi && (!streamId || !accessToken)) {
      setError('Esta transmissão precisa estar vinculada a uma aula cadastrada no sistema.')
      return
    }
  ...
  const persistFinish = async () => {
    if (!usesRemoteSchedulingApi || !streamId || !accessToken) return true
    try {
  ...
  return <div className="studio-shell" role="dialog" aria-modal="true" aria-labelledby="studio-title">
    ...
    <main className="studio-main">
      <section className="studio-preview-panel">
        <div className={`video-stage layout-${layout}`}>
          ...
          {state !== 'paused' && state !== 'ended' && <>
            ...
            {!ready && <div className="stage-message"><span className="stage-icon">▣</span><strong>{hasPersistedClass ? 'Sua prévia aparecerá aqui' : 'Selecione uma aula cadastrada'}</strong><span>{hasPersistedClass ? 'Escolha Somente câmera para testar rapidamente, ou selecione uma tela para compartilhar.' : 'No ambiente conectado à API, o estúdio não pode entrar ao vivo sem uma aula existente no sistema.'}</span></div>}
          </>}
        </div>
      ...
      <aside className="studio-settings">
        ...
        {usesRemoteSchedulingApi && <p className="interaction-note" role="status">{streamId ? `Aula vinculada: ${streamId.slice(0, 8)}…` : 'Nenhuma aula vinculada. Volte ao Painel do criador e escolha Transmitir em uma aula.'}</p>}
        ...
        <small className="broadcast-note">Uma live só passa ao estado AO VIVO depois que a aula vinculada é ativada no backend. A mídia segue diretamente entre os dispositivos durante a homologação.</small>
      </aside>
    </main>
  </div>
}
